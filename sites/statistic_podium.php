<?php
	...
	//driver
	$drivers_old = $db->getDrivers($user['ins_id'], 'AND (f1_podium > 0 OR f2_podium > 0 OR f3_podium > 0)','f1_podium desc, f2_podium desc, f3_podium desc');
	...
	//team
    $teams_old = $db->getTeams($user['ins_id'], 'f1_podium desc, f2_podium desc, f3_podium desc');
    ...
    for($i = 0; $i < $array_size; ++$i)
    {
    	if($league != 0)
			if($teams_old[$i]['league'] != $league)
				continue;
    	if($teams_old[$i]['f1_podium'] > 0 || 
    	   $teams_old[$i]['f2_podium'] > 0 || 
    	   $teams_old[$i]['f3_podium'] > 0)
    	{
    		$_team = $teams_old[$i];
    ...
    $smarty->assign('content', 'statistic_podium.tpl');
