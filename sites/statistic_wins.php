<?php
	...
	//driver
	$drivers_old = $db->getDrivers($user['ins_id'], 'AND (f1_wins > 0 OR f2_wins > 0 OR f3_wins > 0)','f1_wins desc, f2_wins desc, f3_wins desc');
	...
	//team
    $teams_old = $db->getTeams($user['ins_id'], 'f1_wins desc, f2_wins desc, f3_wins desc');
    ...
    for($i = 0; $i < $array_size; ++$i)
    {
    	if($league != 0)
			if($teams_old[$i]['league'] != $league)
				continue;
    	if($teams_old[$i]['f1_wins'] > 0 || 
    	   $teams_old[$i]['f2_wins'] > 0 || 
    	   $teams_old[$i]['f3_wins'] > 0)
    	{
    		$_team = $teams_old[$i];
    ...
    $smarty->assign('content', 'statistic_wins.tpl');
