    		...
    		<div class="col-md-12">
    			<div class="portlet box blue-hoki">
    				<div class="portlet-title">
    					<div class="caption">{translate}tr_statistic_podium_title{/translate}</div>
    					<div class="tools">
    						<div style="float:left">
								<form action="?site=statistic_podium" method="post" style="margin-top: 0px; color:#000">
						...
						<table class="table table-striped">
							...
							{section name=driver loop=$drivers}
								<tr {if $drivers[driver].ok > 0}class="line_highlight"{/if}>
									...
									{/if}
									{if $league < 2}
									<td align="right">{$drivers[driver].f1_podium}</td>
									{/if}
									{if $league == 0 || $league == 2}
									<td align="right">{$drivers[driver].f2_podium}</td>
									{/if}
									{if $league == 0 || $league == 3}
									<td align="right">{$drivers[driver].f3_podium}</td>
									{/if}
								</tr>
							{/section}
							</tbody>
						...
						<table class="table table-striped">
							...
							{section name=team loop=$teams}
								<tr {if $teams[team].id == $team_id}class="line_highlight"{/if}>
									...
									{/if}
									{if $league < 2}
									<td align="right">{$teams[team].f1_podium}</td>
									{/if}
									{if $league == 0 || $league == 2}
									<td align="right">{$teams[team].f2_podium}</td>
									{/if}
									{if $league == 0 || $league == 3}
									<td align="right">{$teams[team].f3_podium}</td>
									{/if}
								</tr>
							{/section}
							</tbody>
